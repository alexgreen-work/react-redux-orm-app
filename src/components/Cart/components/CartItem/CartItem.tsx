...
const CartItem: React.FC<CartItemProps> = ({product, variation, quantity, onRemoveItem, onDecreaseQuantity, onAddQuantity, itemKey}) => {
    const rand = useMemo(()=>Math.random() < 0.5, [])
    const timer = useMemo(()=>moment(), [])
    return (
        <Box className={styles.item} key={itemKey}>
            <Box className={styles.item__image}><img src={cartImage}/></Box>
            <Box className={styles.item__label} sx={{display: 'flex', flexDirection:'column'}}>
            ...
            <Box className={styles.item__count}>
                <Counter value={quantity} handleMinus={()=>onDecreaseQuantity(product.id, variation.id)} handlePlus={()=>onAddQuantity(product.id, variation.id)}/>
            </Box>
            <Box className={styles.item__price}>{variation.price * quantity} ₽</Box>
            <Box className={styles.item__bucket} onClick={()=>onRemoveItem(product.id, variation.id)}><img src={bucketIcon}/></Box>
