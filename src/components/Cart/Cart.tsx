...
const Cart: React.FC = () => {
      ...
      for (const item of cartItems) {
        if (!productsMap[item.productId]) {
          try {
            ...
            console.error(
              `Ошибка загрузки продукта с id ${item.productId}:`,
              error
            );
          }
        }
  ...
  const handleDecreaseQuantity = (productId: number, variationId: number) => {
    dispatch(
      decreaseFromCart({ productId: productId, variationId: variationId })
    );
  };
  ...
  return (
    <Box className={styles.cart}>
      <Box className={styles.header}>
        <Box className={styles.header__label}>Корзина</Box>
          ...
          {cartItems.map((item) => {
            ...
            return (
              <>
                {product && variation ? (
                  <CartItem
                    itemKey={`${product.id}-${variation.id}`}
                  ...
                  <span>
                    Продукт с id {item.productId} не найден{' '}
                    {JSON.stringify(product)} var {JSON.stringify(variation)}
                  </span>
                )}
              </>
            );
